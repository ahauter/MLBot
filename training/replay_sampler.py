...
def _process_one(
    ...
    tiers = [p['rank_tier'] for p in player_ranks if p['rank_tier'] is not None]
    entry = {
        'replay_id':     rid,
...
def collect(
    ...
    to_download = [m for m in replay_list if not (resume and m['id'] in manifest)]
    skipped = len(replay_list) - len(to_download)
    total = len(to_download)
    print(f'Downloading {total} replays ({skipped} already cached) with {workers} worker(s) ...')
