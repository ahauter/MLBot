...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Download and parse Rocket League replays.')
    parser.add_argument('--api-key',    default=os.environ.get('BALLCHASING_API_KEY'), help='ballchasing.com API key (or set BALLCHASING_API_KEY env var)')
    parser.add_argument('--count',      type=int, default=10,              help='Number of replays to collect (default: 10)')
    parser.add_argument('--output-dir', default='training/replay_data',    help='Output directory (default: training/replay_data)')
    parser.add_argument('--min-rank',   default='bronze-1',                help='Minimum rank filter (default: bronze-1)')
    parser.add_argument('--max-rank',   default='bronze-3',                help='Maximum rank filter (default: bronze-3)')
    parser.add_argument('--playlist',   default='ranked-standard',         help='Playlist filter (default: ranked-standard)')
    parser.add_argument('--keep-raw',   action='store_true',               help='Keep raw .replay files after parsing')
    parser.add_argument('--no-resume',  action='store_true',               help='Re-download replays already in manifest')
    args = parser.parse_args()

    if not args.api_key:
        parser.error('--api-key is required (or set BALLCHASING_API_KEY in your environment)')
